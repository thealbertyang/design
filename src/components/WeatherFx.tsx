	...
	useEffect(() => {
		...
		const generateBranch = (
			...
			for (let i = 0; i < branchLength; i++) {
				...
				const segmentLength =
					baseSegmentLength * lengthMultiplier * (0.6 + Math.random() * 0.6)
		...
		const animate = () => {
			...
			if (type === 'rain') {
				...
				particlesRef.current.filter(isLeaf).forEach((leaf) => {
					...
					const scaleX =
						scaleXRaw > 0 ? Math.max(0.15, scaleXRaw) : Math.min(-0.15, scaleXRaw)
				...
				if (canvas && container) {
					...
					if (type === 'rain') {
						...
						for (let i = 0; i < intensity; i++) {
							...
							const color1 = parsedColors[colorIndex]
							const color2 =
								parsedColors[Math.min(colorIndex + 1, parsedColors.length - 1)]
							particles.push({
								x: spawnX,
								y: Math.random() * canvasHeight - canvasHeight,
								width,
